...
if($GLOBALS["MANDRIGO"]["SITE"]["URL_FORMAT"]==1){
    ...
    if(!ereg($GLOBALS["MANDRIGO"]["SITE"]["LOGIN_NAME"]."/",$_SERVER["PHP_SELF"])){
		$php_self=$GLOBALS["MANDRIGO"]["SITE"]["LOGIN_NAME"].$_SERVER["PHP_SELF"];
    }
	else{
		$php_self=$_SERVER["PHP_SELF"];
    }
    //exploes the array into its basic chunks
    $raw_url = eregi_replace("^.*".$GLOBALS["MANDRIGO"]["SITE"]["LOGIN_NAME"]."/a","a",$php_self);

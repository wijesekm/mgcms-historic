...
//{packagename}_vars_hook(&$sql,$i) - anything returned by this will be added to the
//                                                 page_parse_vars when the page class displays the page.
//{packagename}_admin_hook(&$sql,$i) - this will be called by the admin class when it is administering
//                                                 a page with your hook on it
//ALL OTHER FUNCTIONS WILL BE IGNORED
//for the passing in vars $sql is the sql database object,
//$i is the hooks position in the stack of hooks for this page.
//For information on using the $sql please visit the documentation faq.
//
class example_hook{
    function example_display_hook(&$sql,$i){
        $tmp = new example_display();
        return $tmp->ex_display();
    }
    function example_vars_hook(&$sql,$i){
        $tmp = new example_display();
        return $tmp->ex_retvars();
    }
    function example_admin_hook(&$sql,$i){
